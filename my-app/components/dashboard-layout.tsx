...
const roleColors = {
  docente: "bg-blue-50 border-secondary/30",
...
const roleHeaderStyles = {
  docente: "text-primary",
  "admin-bienes": "text-bienes-primary",
  "admin-biblioteca": "text-biblioteca-primary",
}
...
export default function DashboardLayout({ children, role, tabs, activeTab, onTabChange }: DashboardLayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-background flex-col md:flex-row">
      ...
      <aside
        className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-sidebar border-r border-sidebar-border transition-all duration-300 overflow-y-auto flex flex-col ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
        }`}
      >
        <div className="p-4 sm:p-6 border-b border-sidebar-border bg-gradient-to-r from-primary/5 to-accent/5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h2 className="text-base sm:text-xl font-bold text-sidebar-foreground">GestioLibros</h2>
              <p className="text-sidebar-accent text-xs sm:text-xs">GUE</p>
            </div>
          </div>
          <p className="text-sidebar-accent text-xs sm:text-sm">{roleLabels[role]}</p>
        </div>
        ...
        <div className="p-3 sm:p-4 border-t border-sidebar-border">
          <div className="mb-4 text-xs sm:text-sm">
            <p className="text-sidebar-accent">Conectado como:</p>
            <p className="text-sidebar-foreground font-medium truncate">{user?.name}</p>
          </div>
          <Button
            variant="outline"
            className="w-full justify-start text-xs sm:text-sm text-sidebar-foreground hover:bg-sidebar-accent bg-transparent"
            onClick={() => {
              logout()
      ...
      <main className="flex-1 overflow-hidden flex flex-col w-full">
        <header className={`border-b px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between ${roleColors[role]}`}>
          <Button variant="ghost" size="sm" className="md:hidden" onClick={() => setSidebarOpen(!sidebarOpen)}>
            {sidebarOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
          </Button>
          <h1 className={`text-lg sm:text-2xl font-bold ${roleHeaderStyles[role]}`}>{roleLabels[role]}</h1>
          <div className="w-10 md:w-12" />
        </header>

        <div className={`px-3 sm:px-6 py-3 sm:py-4 border-b ${roleColors[role]} overflow-x-auto`}>
